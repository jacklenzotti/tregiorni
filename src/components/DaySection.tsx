import { dayCost, formatBand } from '../domain/budget/estimateCost';
...
export function DaySection(props: DaySectionProps) {
  const { day, settings, stops, usedIds, selectedStopId, dispatch } = props;
  const budget = dayCost(stops.map((stop) => stop.place));
  return (
    <section className={`day day-${day + 1}`}>
      <header className="day-header">
        <h2>
          Day {day + 1} <span className="weekday">{WEEKDAY_NAMES[weekdayOfDay(settings, day)]}</span>
          {stops.length > 0 && <span className="day-budget">{formatBand(budget)}</span>}
        </h2>
        <div className="day-actions">
          <button onClick={() => dispatch({ type: 'AUTO_FILL_DAY', day })}>Auto-fill day</button>
          <button
            className="ghost"
